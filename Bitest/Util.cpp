...
float CUtil::KmeterToOther(float values,CString strUnit)
{
...
CString CUtil::ReadLangString(const CString& strSection,const CString strKey)
{
	CString strVal;
	CString strFile = GetLangFilePath();

	GetPrivateProfileString(strSection, strKey, _T(""), strVal.GetBuffer(MAX_PATH), MAX_PATH, strFile);
	strVal.ReleaseBuffer();

	return strVal;
}

CString CUtil::ReadConfigInfo(const CString& strSection, const CString strKey)
{
	CString strVal;
	GetPrivateProfileString(strSection, strKey, _T(""), strVal.GetBuffer(MAX_PATH), MAX_PATH, GetExePath()+ CONFIG_FILE);
	strVal.ReleaseBuffer();

	return strVal;
}

bool CUtil::WriteConfigInfo(const CString& strSection, const CString& strKey, const CString& strVal)
{
	return WritePrivateProfileString (strSection, strKey, strVal, GetExePath()+ CONFIG_FILE);
}

CString CUtil::GetExePath()//获取文件运行路径
{
	CString sPath;
	GetModuleFileName(NULL,sPath.GetBufferSetLength(MAX_PATH+1),MAX_PATH);
	sPath.ReleaseBuffer();
...
CString CUtil::GetLangFilePath()
{
	CString strFilePath;

	if (g_iLang == LANG::CHS)
		strFilePath.Format(_T("%s%s"),GetExePath(),  CHS_FILE);
	else if (g_iLang == LANG::ENG)
		strFilePath.Format(_T("%s%s"),GetExePath(),  ENG_FILE);
	else 
		;
