...
//	g_gaborDllUtil.Acount_Total_Loss(testResultInfo.WaveLength, testResultInfo.PulseWidth, testResultInfo.pDataPoints, &gaborResult, testResultInfo.Refraction, testResultInfo.Frequency);
	for(int i=0;i<nTemp;i++)
	{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//启动光纤
	//解析LPCSTR strAllOther;//除了上面参数外，其它参数，以不用每次修改都要考虑是否兼容//暂时不弄
	//返回厂商名称等 wcq2015.10.27
	if (sorData.vParams.size()>0)
	{
		//每一个CString格式如 name=grandway;age=20
		INT nCount = sorData.vParams.size();
		CStringArray strArr;
		CString strKey,strValue,temp;
		char chArr[256];//不考虑更长的字符串
		for (INT i=0; i<nCount; i++)
		{
			temp = sorData.vParams[i];
			CUtil::splitString(temp, '=', strArr);//调用了，util类!!
			if (strArr.GetSize() < 2)
			{
				continue;
			}//eof
			strKey = strArr[0];
			strValue = strArr[1];
			//if (_T("name") == strKey)//m_suppliersParas.name
			//{
			//	nTemp = UnicodeToANSIEn(strValue, chArr);
			//	chArr[nTemp - 1] = '\0';
			//	m_suppliersParas.name.assign(chArr, chArr+nTemp);
			//}
			if (_T("UO") == strKey)//"User Offset"
			{
				if (_T("") != strValue)
				{
					//testResultInfo.UserOffset = (_ttoi(strValue) * C_LIGHT_SPEED)/(testResultInfo.Refraction *pow(10.0,10));
				}//eif
			}
			else if (_T("UOD") == strKey)//"User Offset Distance" 103表示10.3m
			{
				if (_T("") != strValue)
				{
					//testResultInfo.UserOffsetDistance = _ttoi(strValue)/ 100.0;//暂时不用
				}//eif
			}//eif
			else if (_T("AveragingTime") == strKey)//"User Offset Distance" 103表示10.3m
			{
				if (_T("") != strValue)
				{
				//	testResultInfo.nAveragingTime = _ttoi(strValue);//暂时不用//平均时间 wcq2016.03.10
				}//eif
			}//eif
		}//efor
	}//eif
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	testResultInfo.strTestTime = ctime(&sorData.TestTimestamp);//增加测试时间
}
...
void CSorAssist::convert(TestResultInfo* testResultInfo, SORDATA& sorData)
{
	...
	//启动光纤
//	CString temp;
//	if (0 != testResultInfo->UserOffset)
	{
	//	INT nUO = (testResultInfo->UserOffset* (testResultInfo->Refraction *pow(10.0,10)))/ C_LIGHT_SPEED+0.5;//0.5:四舍五入,精度:0.02m,即2cm
	//	FLOAT ff=(nUO * C_LIGHT_SPEED)/(testResultInfo->Refraction *pow(10.0,10));
	//	temp.Format(_T("UO=%d"), nUO);
	//	sorData.vParams.push_back(temp);
	}//eif
	//temp.Format(_T("AveragingTime=%d"), testResultInfo->nAveragingTime);//平均时间 wcq2016.03.10
	//sorData.vParams.push_back(temp);
...
//	element.SetParent(pOtdrTrace);//wcq2016.04.27
...
//	element.SetParams(testResultInfo.WaveLength, testResultInfo.PulseWidth, testResultInfo.BackscatterCoefficient);//wcq2016.04.29
//	element.m_bShowEvent = pOtdrTrace->m_bShowEventList;//wcq2015.05.06
//	element.m_userOffsetDistance = GetUserOffset(sorData);//wcq2015.12.07
	pOtdrTrace->m_vAllData.push_back(element);
//	pOtdrTrace->SetMaxDistance(element.m_fLength);
	pOtdrTrace->SetScrollRangeX();//
	////pOtdrTrace->SetRange(0, element.m_fLength, 0, 65.535);////cwcq2016.01.29
	pOtdrTrace->SetRange(0, element.m_fLength, 0, OTDR_TRACE_Y_MAX);////wcq2016.01.29
...
// Access:    public 
// Returns:   void
// Parameter: TestResultInfo * pTestResultInfo
// Parameter: COtdrTrace * pOtdrTrace
// Parameter: BOOL bShowOnlyOne
// Parameter: WORD * pDataSmooth
// Parameter: COLORREF colorLine
// Comment(wcq2016-1-27):将读取到的sor文件数据添加到显示列表
////////////////////////////////////////////////////////////////////////////////////////////////////////
void CSorAssist::AddSordataToOtdrTrace( TestResultInfo* pTestResultInfo, COtdrTrace* pOtdrTrace, BOOL bShowOnlyOne/*=FALSE*/, COLORREF colorLine/*=RGB(0, 0, 255)*/ , WORD* pDataSmooth/*=NULL*/)
{
#ifdef COMPILE_SHOW_SMOOTH_DATA////显示时用平滑过的数据，保存时用实际数据pDataPointsSmooth wcq2013.04.26
	std::vector<FLOAT> recordSmooth;//
#endif
	...
	int countEachFactor;
	INT nDataCount = pTestResultInfo->DataPointsNum;
	WORD* pData = pTestResultInfo->pDataPoints;
	//将读到的数据放到m_record中

	std::vector<FLOAT> record;//
	for (int j=0;j<nDataCount;j++)
	{
		record.push_back((65535-(int)pData[j])/1000.0);
	}//efor

#ifdef COMPILE_SHOW_SMOOTH_DATA////显示时用平滑过的数据，保存时用实际数据pDataPointsSmooth wcq2013.04.26
	for (int i=0;i<nDataCount;i++)
	{
		recordSmooth.push_back((65535-(int)pData[i])/1000.0);
	}//efor
#endif
	...
	if (bShowOnlyOne && (pOtdrTrace->m_vAllData.size()>0))
	{
		element = &pOtdrTrace->m_vAllData[0];
	//	element->Release();//先释放原来new的空间
	}
	else
	{
		element = new CGraphElement();
		bNewElement = TRUE;
	}//eif
//	element->SetParent(pOtdrTrace);//wcq2016.04.27
	element->m_nPointsCount = pTestResultInfo->DataPointsNum;
	element->m_fLength = pTestResultInfo->Length;//zll 2013.4.23改为如下
	element->m_fDataSpacing = pTestResultInfo->DataSpacing;
	element->m_LineColor = colorLine;
	element->m_pData = new FLOAT[element->m_nPointsCount];
	INT nCount = element->m_nPointsCount;
	...
	memcpy(element->m_pData, &record[0], nCount * sizeof(FLOAT));
	SetEvents(*pTestResultInfo, element);
//	element->SetParams(pTestResultInfo->WaveLength, pTestResultInfo->PulseWidth, pTestResultInfo->BackscatterCoefficient);//wcq2016.04.29
//	element->m_bShowEvent = pOtdrTrace->m_bShowEventList;//wcq2015.05.06
//	element->m_userOffsetDistance = pTestResultInfo->UserOffsetDistance;//wcq2015.12.07
	if (bNewElement)
	{
		pOtdrTrace->m_vAllData.push_back(*element);
	}//
	...
	////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////
	//////pOtdrTrace->SetMaxDistance(element->m_fLength);//cwcq2016.01.29
	//////pOtdrTrace->SetScrollRangeX();////cwcq2016.01.29
	//////pOtdrTrace->SetRange(0, element->m_fLength, 0, 65.535);////cwcq2016.01.29
	//////pOtdrTrace->OnInitAB();  //lzy2014.9.10//cwcq2016.01.29
//	pOtdrTrace->m_bModeShowOne = bShowOnlyOne;//wcq2016.01.29
	INT nCurSele = 0;
	if (!bShowOnlyOne)
	{
		nCurSele = pOtdrTrace->m_vAllData.size()-1;
	}
	else
	{
//		pOtdrTrace->SetMaxDistance(element->m_fLength);//, OTDR_TRACE_Y_MIN, OTDR_TRACE_Y_MAX);////wcq2016.02.01
	}//eif
//	pOtdrTrace->SetCurrentSeleTrace(nCurSele, pTestResultInfo->pDataPoints);

	//pOtdrTrace->DrawCurves(&pOtdrTrace->m_dcCurve,-1);//cwcq2016.01.29
	//
	if (bNewElement)
	{
		delete element;//不能调用Release()???
	}//
	record.clear();
}
...
//colorLine:曲线颜色
void CSorAssist::CloseCurvedata(int nListItem,SORDATA sorData, COtdrTrace* pOtdrTrace, COLORREF colorLine)
{
	int temp=-1;
	...
	for (;it!=pOtdrTrace->m_vAllData.end();it++)
	{
		...
		if (nListItem == temp)
		{
		//	it->Release();//wcq2016.03.16
			pOtdrTrace->m_vAllData.erase(it);
			break;
		}
	}
//	pOtdrTrace->TraceCountChanged();//wcq2016.03.16
	//pOtdrTrace->DrawCurves(&pOtdrTrace->m_dcCurve,-1);//cwcq2016.03.16
...
//	element.SetParent(pOtdrTrace);//wcq2016.04.27
	...
	////pOtdrTrace->SetCurrentSeleTrace(pOtdrTrace->m_vAllData.size()-1, testResultInfo.pDataPoints);//cwcq2016.04.28
//	pOtdrTrace->SetCurrentSeleTrace(pOtdrTrace->m_vAllData.size()-1, testResultInfo.pDataPoints);//wcq2016.04.28
	//pOtdrTrace->DrawCurves(&pOtdrTrace->m_dcCurve);
	//
	record.clear();
}
...
// Access:    public 
// Returns:   FLOAT
// Parameter: SORDATA & sorData
// Comment(wcq2015-12-7):得到启动光纤长度
////////////////////////////////////////////////////////////////////////////////////////////////////////
FLOAT CSorAssist::GetUserOffset( SORDATA &sorData )
{
	FLOAT fResult = 0;
	if (sorData.vParams.size()>0)
	{
		...
		for (INT i=0; i<nCount; i++)
		{
			...
			if (_T("UO") == strKey)//"User Offset"
			{
				if (_T("") != strValue)
				{
					fResult = (_ttoi(strValue) * C_LIGHT_SPEED)/(sorData.Refraction *pow(10.0,10));
				}//eiff
			}//eif
		}//efor
	}//eif
