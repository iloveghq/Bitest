...
void CPrintView::DrawPrintContent(CDC* pDC, CRect rectDraw, INT nCurSel, BOOL bSingle)
{
	...
	if(m_bPrintQuest)
	{
		...
		strText = m_stPrintDetail.stPrintQuest.strSorFileName + _T(": ") + m_stPrintDetail.stPrintQuest.SorFileName;  //写文件名
		sizeText = pDC->GetTextExtent(strText);
		...
		strText = m_stPrintDetail.stPrintQuest.strTestDate + _T(": ") + m_stPrintDetail.stPrintQuest.TestDate;  //写测试时间
		sizeText = pDC->GetTextExtent(strText);
...
void CPrintView::DrawEventHead(CDC* pDC, INT nLeft, INT nTop, CRect rectDraw,INT iCurSel)//绘制事件列表
{
...
Write By:             XJF                                                                                          
****************************************************************/
void CPrintView::DrawEventList(CDC* pDC, INT nLeft, INT nTop , INT nVspace , CRect rectDraw, INT nCurSel)//绘制事件列表
{
	CFont* pOldFont = pDC->SelectObject (&m_dataFont);
	INT nLeftSpace = rectDraw.Width() / MARGIN_LEFT_COUNT;
...
Write By:             ghq                                                                                          
****************************************************************/
void CPrintView::DrawEventList2Way(CDC* pDC, INT nLeft, INT nTop , INT nVspace , CRect rectDraw, INT nCurSel)//绘制事件列表
{
	CFont* pOldFont = pDC->SelectObject (&m_dataFont);
	INT nLeftSpace = rectDraw.Width() / MARGIN_LEFT_COUNT;
...
void CPrintView::DrawEvent(CDC* pDC, CRect& rect, INT nCurveIndex)
{
	...
	{		
		...
		fY=INT(rect.bottom - (65535-pData->pDataPoints[pData->EventList.vEventInfo[i].index])/1000.0*rect.Height()/(SET_LOSS_MAX-OTDR_TRACE_Y_MIN));//lzy2014.7.23 使事件点文字不沿y轴移动
...
void CPrintView::DrawCurve2Way(CDC* pDC, CRect& rect, INT nCurveIndex)
{
	...
	INT nPointsTotal = m_stPrintDetail.stTestResultInfo[0].DataPointsNum + nPointsStart;   //数据总数
...
Write By:             ghq                                                                                          
****************************************************************/
void CPrintView::DrawEventList2WayPage2(CDC* pDC, INT nLeft, INT nTop , INT nVspace , CRect rectDraw, INT nCurSel)//绘制事件列表
{
	CFont* pOldFont = pDC->SelectObject (&m_dataFont);
	INT nLeftSpace = rectDraw.Width() / MARGIN_LEFT_COUNT;
	INT nHspace = (rectDraw.Width() - 2 * nLeftSpace) / 7;  //lzy2014.8.12
...
Write By:             XJF                                                                                          
****************************************************************/
void CPrintView::DrawEventListPage2(CDC* pDC, INT nLeft, INT nTop , INT nVspace , CRect rectDraw, INT nCurSel)//绘制事件列表
{
	CFont* pOldFont = pDC->SelectObject (&m_dataFont);
	INT nLeftSpace = rectDraw.Width() / MARGIN_LEFT_COUNT;
	INT nHspace = (rectDraw.Width() - 2 * nLeftSpace) / EVENT_LIST_COL;  //lzy2014.8.12
