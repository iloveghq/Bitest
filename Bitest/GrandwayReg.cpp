...
void CGrandwayReg::writeFile(CString strPath, CString exePath)//写文件
{
	int ilen = strPath.GetLength();
	//exePath.Replace(_T("\\"),_T("\\\\"));
	exePath = exePath + CONFIG_FILE;
	int nPos;
	nPos=strPath.ReverseFind('\\');
	strPath=strPath.Left(nPos);
    ::WritePrivateProfileString(_T("FileOpen"),_T("path"),strPath,exePath);
}

void CGrandwayReg::readFile(CString strPath, CString exePath)//读文件
{
	LPWSTR lpStr = (LPWSTR)(LPCTSTR)strPath;
	//exePath.Replace(_T("\\"),_T("\\\\"));
	exePath = exePath + CONFIG_FILE;
    ::GetPrivateProfileString(_T("FileOpen"),_T("path"),NULL,lpStr,MAX_PATH,exePath);
}
